...
Feature: Hotel Room Booking
  ...
  Scenario Outline: Successful hotel room booking creation, modification and cancellation
    Given the user submits valid login credentials:
      | username | password |
      | admin    | password |
    Then the system should authenticate the user
    And the user should receive a valid session
    Then user gives checkin date as "<checkin>" and checkout date as "<checkout>"
